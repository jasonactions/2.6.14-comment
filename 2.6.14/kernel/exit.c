...
static void exit_mm(struct task_struct * tsk)
{
	...
	/*如果多个共享内存描述符的轻量级进程正在将内存中内容转储到文件*/
	if (mm->core_waiters) {
		up_read(&mm->mmap_sem);
	...
	/*由于进程释放是在context_switch新进程之后，inc是为了避免mm被释放,因为后面还会用到*/
	...
	task_unlock(tsk);
	/*
	 * 释放局部描述符表、线性区和页表,由于前面递增了mm->mm_count，因此不会释放mm
	 * 由finish_task_switch函数释放
	 */
	mmput(mm);
}
