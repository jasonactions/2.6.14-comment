				...
				struct address_space *mapping)
{
	...
	/*获取bio_vec位于所在page的offset*/
	unsigned int vec_offset = vec->bv_offset;
	/*获取sector所在block的偏移(字节)*/
	...
	do {
		...
		/*如果当前block空闲空间count大于要读写的bio_vec大小size，则一次循环操作即可*/
...
static int rd_make_request(request_queue_t *q, struct bio *bio)
{
	...
	/*从起始sector开始，如果要读写的数据量超过了ramdisk的容量则退出*/
	if (sector + len > get_capacity(bdev->bd_disk))
		goto fail;

	if (rw==READA)
		rw=READ;
	/*遍历bio下的每一个bio_vec，对其进行读/写 from/to ramdisk*/
...
/*
 * @inode: 主inode,它一般通过bdget时创建
 *
 * 打开/dev/ram设备节点的时候会调用，通过vfs_open->blkdev_open调用过来
 * 它重现赋值了inode->mapping->a_ops = &ramdisk_aops
 */
static int rd_open(struct inode *inode, struct file *filp)
{
	...
	if (rd_bdev[unit] == NULL) {
		...
		/*
		 * 在执行rd_open之前，bdget在主inode->i_data->a_ops初始化为def_blk_aops，
		 * 在执行rd_open之后，主inode->i_data->a_ops更改为ramdisk_aops
		 */
...
static int __init rd_init(void)
{
	...
	/*注册block device,实际是向major_names数组添加一项*/
	...
	for (i = 0; i < CONFIG_BLK_DEV_RAM_COUNT; i++) {
		...
		if (!rd_queue[i])
			goto out_queue;
		/*设置request_queue->make_request_fn为rd_make_request,之后将对gendisk进行基本的初始化*/
		...
		disk->first_minor = i;
		/*rd_bd_op为block_dev_ops，它将在def_blk_ops中被调用*/
		disk->fops = &rd_bd_op;
