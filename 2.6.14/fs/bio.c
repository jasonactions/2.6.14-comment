 ...
 */
/*
 * 创建BIOVEC_NR_POOLS个内存池，每个内存池有pool_entries个元素
 */
...
/*
 * 创建一个bio内存池，元素个数为bio_pool_size
 * 创建biovec内存池,每个内存池的元素个数为bvec_pool_size
 */
struct bio_set *bioset_create(int bio_pool_size, int bvec_pool_size, int scale)
{
	...
	/*为bio创建一个内存池，元素个数为bio_pool_size*/
	bs->bio_pool = mempool_create(bio_pool_size, mempool_alloc_slab,
			mempool_free_slab, bio_slab);

	if (!bs->bio_pool)
		goto bad;
	/*为bio_vec创建多个内存池，每个内存池包含bvec_pool_size个元素*/
