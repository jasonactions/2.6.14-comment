...
/*根据块设备号查询bdev文件系统的inode哈希表获取inode,通过container_of返回bdev_inode->bdev*/
struct block_device *bdget(dev_t dev)
{
	struct block_device *bdev;
	struct inode *inode;
	/* 
	 * 尝试从 bdev文件按系统的inode cache获取inode，如果不存在则需要重新分配
	 * 它会调用bdev文件系统的bdev_sops->alloc_inode，创建bdev_inode
	 *
	 * note: 所有的inode都将通过一个hash list进行管理,对bdev文件系统，是通过将设备号通过计算得到哈希值
	 */
	...
	if (inode->i_state & I_NEW) {
		...
		inode->i_rdev = dev;
		inode->i_bdev = bdev;
		/*设置inode->address_space.a_ops为def_blk_aops,具体的块设备可以在执行open时进行更改*/
		inode->i_data.a_ops = &def_blk_aops;
		mapping_set_gfp_mask(&inode->i_data, GFP_USER);
		/*设置inode->address_space.bdi为default_backing_dev_info*/
