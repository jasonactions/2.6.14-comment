...
/*
 * @filename:要打开文件路径名
 * @flags: 访问模式标志位,如：O_RDONLY,O_WRONLY,O_RDWR...
 * @mode:  文件创建许可权限位掩码
 * @returun: 成功返回文件描述符，即current->files->fd的索引，否则返回-1
 */
...
asmlinkage long sys_close(unsigned int fd)
{
	...
	if (!filp)
		goto out_unlock;
	/*将current->files->fd[fd]设为NULL*/
	rcu_assign_pointer(fdt->fd[fd], NULL);
	/*清除current->files->close_on_exec*/
	FD_CLR(fd, fdt->close_on_exec);
	/*清除current->files->open_fds*/
	__put_unused_fd(files, fd);
	spin_unlock(&files->file_lock);
